...
public class TiffReader
{
                            ...
                            @NotNull final TiffHandler handler) throws TiffProcessingException, IOException
    {
        ...
        switch (tiffStandard)
        {
	        case TIFF:
	        	isBigTiff = false;
	        	firstIfdOffset = reader.getInt32(4);
        ...
        TiffReaderContext context = new TiffReaderContext(reader, reader.isMotorolaByteOrder(), isBigTiff);
    ...
    public static void processIfd(@NotNull final TiffHandler handler,
                                  @NotNull final TiffReaderContext context,
                                  final int ifdOffset) throws IOException
    {
    	...
    	final Set<Integer> processedIfdOffsets = null; // FIXME
    	final boolean isBigTiff = context.isBigTiff();
        ...
        try {
                ...
                if (byteCount == 4 * componentCount || formatCode == TiffDataFormat.CODE_IFD8) {
                    for (long i = 0; i < componentCount; i++) {
                        if (handler.tryEnterSubIfd(tagId)) {
                            ...
                            processIfd(handler, context, subDirOffset, tiffHeaderOffset);
                        }
                    }
                }

                // If it wasn't an IFD pointer, allow custom tag processing to occur
                if (!isIfdPointer && !handler.customProcessTag(context, tiffHeaderOffset, tagId, (int) byteCount)) {
            ...
            if (nextIfdOffsetLong != 0 && nextIfdOffsetLong <= Integer.MAX_VALUE) {
            	int nextIfdOffset = (int) nextIfdOffsetLong + tiffHeaderOffset;
                ...
                if (handler.hasFollowerIfd()) {
                    processIfd(handler, context, nextIfdOffset, tiffHeaderOffset);
                }
            }
        } finally {
            handler.endingIFD();
